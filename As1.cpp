...
#pragma omp parallel num_threads(totlathreads)
    {
        int thread_id = omp_get_thread_num();
        int start = thread_id * local_vsize;
        int end = (thread_id == totlathreads - 1) ? vsize : start + local_vsize;
...
void measure_thread_count_vs_time(const std::vector<int>& vector1, const std::vector<int>& vector2, std::vector<int>& result, int amax_threads) {
    ...
    std::cout << "\n\nThread Count vs. Time:\nMaximum available threads in the system is " << max_threads << "\n" << std::endl;
